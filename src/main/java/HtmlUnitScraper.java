...
//                    ex.printStackTrace();
    ...
    private static void getTransferStatsNew(HtmlPage htmlPage, PlayerRepository playerRepository) {
        ...
        for (final HtmlTableRow row : table.getRows().subList(1, table.getRowCount())) {
            ...
            String preferredFoot = row.getCell(6).asNormalizedText();
            String contractEndDate = row.getCell(9).asNormalizedText();
            int marketValue = calculateValue(row.getCell(10).asNormalizedText());
            int height = calculateHeight(row.getCell(5).asNormalizedText());
...
//            LocalDate dateOfBirth = LocalDate.parse(dateOfBirthString.split("\\(")[0], formatter);
    ...
    public static void getStandardStatsNew(HtmlPage htmlPage, PlayerRepository playerRepository) {
        ...
        for (final HtmlTableRow row : table.getRows().subList(2, table.getRowCount() - 2)) {
            ...
            String playerAgeString = row.getCell(3).asNormalizedText();

            int playerAge = 0;

            try{
                playerAge = Integer.parseInt(playerAgeString);
            } catch(Exception e){
                e.printStackTrace();
            }
            ...
            int finalPlayerAge = playerAge;
            player.ifPresent(p -> {
                p.setPlayerNation(playerNation);
                p.setPlayerAge(finalPlayerAge);
